...
try {
  ...
  console.error('sdk-missing: @larksuiteoapi/node-sdk not found (ensure it is installed in the profile node_modules)');
  process.exit(1);
}
...
try {
  const wsClient = new lark.WSClient({
    ...
    onError: function (e) { logError('ws-error:' + (e && e.message ? e.message : JSON.stringify(e))); }
  });
