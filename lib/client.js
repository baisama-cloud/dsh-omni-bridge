window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function usersText(list){return Array.isArray(list)?list.join(','):'';}
    function patchUsers(patch, path, value){patch(path, String(value||'').split(',').map(function(s){return s.trim();}).filter(Boolean));}
    ...
    function BridgeSettings(props){
      ...
      React.useEffect(function(){let alive=true;call('GET','/omni-bridge/get-config').then(function(cfg){if(alive&&cfg){setConfig(cfg);setStatus({kind:'ok',msg:'已加载'});}else if(alive){setStatus({kind:'error',msg:'配置为空'});}}).catch(function(e){if(alive)setStatus({kind:'error',msg:'加载失败: '+(e&&e.message)});});return function(){alive=false;};},[]);
      function patch(path,value){setConfig(function(prev){const next=JSON.parse(JSON.stringify(prev));const keys=path.split('.');let node=next;for(let i=0;i<keys.length-1;i++)node=node[keys[i]];node[keys[keys.length-1]]=value;call('POST','/omni-bridge/set-config',next).catch(function(){});return next;});}
      function testChannel(id){setBusy(true);setStatus({kind:'busy',msg:'测试 '+id+'…'});call('POST','/omni-bridge/bridge-send',{channel:id,text:'桥接测试消息',target:'',contextToken:'',msgId:'',isGroup:false}).then(function(r){setBusy(false);setStatus(r&&r.ok?{kind:'ok',msg:id+' 发送成功'}:{kind:'error',msg:id+'：'+((r&&r.error)||'失败')});}).catch(function(e){setBusy(false);setStatus({kind:'error',msg:'测试失败'});});}
      function getQr(){setBusy(true);call('GET','/omni-bridge/weixin-qr').then(function(r){setBusy(false);if(r&&r.ok){setQr({qrcode:r.qrcode,url:r.url});setStatus({kind:'ok',msg:'二维码已生成，请扫码'});}else{setStatus({kind:'error',msg:'获取失败: '+((r&&r.error)||'')});}}).catch(function(e){setBusy(false);setStatus({kind:'error',msg:'获取失败'});});}
      function pollQr(){if(!qr||!qr.qrcode){setStatus({kind:'error',msg:'请先获取二维码'});return;}setBusy(true);call('POST','/omni-bridge/weixin-poll',{qrcode:qr.qrcode}).then(function(r){setBusy(false);if(r&&r.ok&&r.botToken){setQr(null);setStatus({kind:'ok',msg:'登录成功 ✓'});call('GET','/omni-bridge/get-config').then(function(c){if(c)setConfig(c);});}else{setStatus({kind:'busy',msg:'状态: '+((r&&r.status)||'wait')});}}).catch(function(e){setBusy(false);setStatus({kind:'error',msg:'查询失败'});});}
      function restartQQ(){setBusy(true);call('POST','/omni-bridge/qq-restart').then(function(){setBusy(false);setStatus({kind:'ok',msg:'QQ 网关已重启'});}).catch(function(){setBusy(false);setStatus({kind:'error',msg:'重启失败'});});}
      function restartFeishu(){setBusy(true);call('POST','/omni-bridge/feishu-restart').then(function(){setBusy(false);setStatus({kind:'ok',msg:'飞书网关已重启'});}).catch(function(){setBusy(false);setStatus({kind:'error',msg:'重启失败'});});}
      ...
      return React.createElement('div',{className:'ob-root'},
        ...
        React.createElement(Card,{title:'微信 · ClawBot（iLink）',defaultOpen:true,onTest:function(){testChannel('weixin');}},
          ...
          React.createElement(TextField,{label:'允许的用户 ID（逗号分隔，留空=关闭）',value:usersText(wx.allowedUsers),onChange:function(v){patchUsers(patch,'channels.weixin.allowedUsers',v);}}),
          React.createElement(Toggle,{label:'允许所有人（allowAll）',value:wx.allowAll,onChange:function(v){patch('channels.weixin.allowAll',v);}}),
        ...
        React.createElement(Card,{title:'QQ 机器人（官方开放平台）',defaultOpen:true},
          ...
          React.createElement(TextField,{label:'允许的用户 ID（逗号分隔，留空=关闭）',value:usersText(qq.allowedUsers),onChange:function(v){patchUsers(patch,'channels.qq.allowedUsers',v);}}),
          React.createElement(Toggle,{label:'允许所有人（allowAll）',value:qq.allowAll,onChange:function(v){patch('channels.qq.allowAll',v);}}),
        ...
        React.createElement(Card,{title:'飞书 Bot（自建应用）',defaultOpen:true},
          ...
          React.createElement(TextField,{label:'允许的用户 ID（逗号分隔，留空=关闭）',value:usersText(feishu.allowedUsers),onChange:function(v){patchUsers(patch,'channels.feishu.allowedUsers',v);}}),
          React.createElement(Toggle,{label:'允许所有人（allowAll）',value:feishu.allowAll,onChange:function(v){patch('channels.feishu.allowAll',v);}}),
    ...
    function apply(ctx) {
      ...
      style.setAttribute('data-plugin', 'dsh-omni-bridge');
